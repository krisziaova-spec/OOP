...
CREATE TABLE IF NOT EXISTS requirement_due_rule (
    rule_id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    requirement_type_id INT NOT NULL UNIQUE,
    first_due_days INT NOT NULL DEFAULT 30,
    resend_after_days INT NOT NULL DEFAULT 30,
    rule_is_active SMALLINT NOT NULL DEFAULT 1,
    CONSTRAINT fk_requirement_due_rule_type
        FOREIGN KEY (requirement_type_id)
        REFERENCES requirement_type(type_id)
);
...
CREATE TABLE app_user (
    ...
    user_photo_url     TEXT,
    user_photo_storage_path VARCHAR(500),
...
CREATE TABLE applicant (
    ...
    applicant_educational_background   VARCHAR(150),
    applicant_is_protected             SMALLINT     NOT NULL DEFAULT 1,
    applicant_is_deleted               SMALLINT     NOT NULL DEFAULT 0,
    applicant_deleted_at               TIMESTAMP,
...
CREATE UNIQUE INDEX ux_applicant_email_except_dummy
    ON applicant (LOWER(applicant_email_address))
    WHERE LOWER(applicant_email_address) <> 'krisziamaehpamintuan@iskolarngbayan.pup.edu.ph';
...
CREATE INDEX idx_requirement_storage_path ON requirement(requirement_storage_path);

CREATE INDEX idx_req_tracking_no
    ON requirement(requirement_tracking_no);

CREATE UNIQUE INDEX ux_requirement_application_type
    ON requirement(application_id, requirement_type_id);
...
-- =====================================================
-- LOCAL PERMISSIONS
-- Safe for local PostgreSQL setup using database pdts_db and user pdts_user.
-- =====================================================

GRANT ALL PRIVILEGES ON DATABASE pdts_db TO pdts_user;
GRANT USAGE, CREATE ON SCHEMA public TO pdts_user;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO pdts_user;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO pdts_user;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT ALL ON TABLES TO pdts_user;

ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT ALL ON SEQUENCES TO pdts_user;
